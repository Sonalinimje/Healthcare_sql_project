...
select a.appointment_id, p.first_name as patient_first_name, p.last_name as patient_last_name,d.first_name as doctor_first_name,
d.last_name as doctor_last_name,b.amount,b.payment_date,b.status
from appointments a 
...
-- retrive patients who had appointments in the last 30 days
...
from appointments group by year order by year;

-- indentify the most frequently prescribed medications and their total dosage
select medication,count(*) as frequency,sum(cast(substring_index(dosage,' ',1) as unsigned)) as total_dosage
from prescriptions group by medication order by frequency desc;
...
from appointments group by appointment_date order by appointment_count desc;
...
-- find all appointments for docator with id 1
...
join patients p on a.patient_id = p.patient_id
where a.appointment_date between '2024-08-01' and '2021-08-10';
